...
export const Nav = ({ selected }: { selected: string }) => (
  <div className="flex justify-center fixed inset-x-0 bottom-0">
    <nav className="w-full max-w-md left-0 right-0 bottom-0 px-4 pb-5">
      <div className="w-full grid grid-cols-5 py-2 px-7 rounded-full shadow-float bg-white">
        {ROUTERS.map((item, index) => (
          ...
          >
            ...
            <div className="text-[11px] mt-1 font-medium z-[1]">
              {item.text}
            </div>
          </Link>
        ))}
